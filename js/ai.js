...
const AI = {
  ...
  parseAction(raw) {
    ...
    // Pull out the LLM-generated x (random percent in [1, 3]). Accept
    // "X: 2", "X: 2.0%", "X = 0.02" — anything we can pluck a number
    // from. Out-of-range or missing values fall back to a midpoint
    // 2% downstream so the order still has a sensible price.
    let x = null;
    const xm = raw.match(/\bX\s*[:=]\s*([0-9]+(?:\.[0-9]+)?)\s*%?/i);
    if (xm) {
      const v = parseFloat(xm[1]);
      if (Number.isFinite(v)) x = v;
    }
    return { action, reason: rm ? rm[1].trim() : '', x };
  },
  ...
  async getPlanBeliefs(agents, market, config, aiCfg, plan, tunables, logger, regulator) {
    ...
    const promptFor = (a) => {
      ...
      // BID/ASK_1 prices are parametric on the LLM's own draw of x
      // (a random percent in [1, 3]); we cannot pre-check affordability
      // against an unknown price, so the per-action gating below uses
      // best_bid / best_ask as a permissive lower bound for cash and
      // the inventory check for ASK_1. The actual order price is
      // recomputed in agents.js once x lands.
      const actions = [];
      const constraints = [];
      ...
      if (cash > 0) {
        actions.push(`3. BID: Submit bid = best_bid*x.`);
      } else {
        constraints.push(`- BID cannot be selected (cash ${cash} ≤ 0).`);
      }
      if (inv >= 1) {
        actions.push(`4. ASK_1: Submit ask = best_ask/x.`);
      } else {
        constraints.push(`- ASK_1 cannot be selected (holdings < 1).`);
      }
      ...
      lines.push(
        ...
        `3. Holding too many positions increases inventory risk;`,
        ``,
      ...
      lines.push(
        ...
        `0. First, generate a random percentage between 1% and 3%, x.`,
        ...actions,
      ...
      if (constraints.length) {
        ...
        lines.push(
          `- If the price generated by ASK_1 is <= best_bid, it is equivalent to a sell order that will be executed immediately.`,
          `- If the price generated by BID is >= best_ask, it is equivalent to a buy order that will be executed immediately.`,
        );
      }
      ...
      lines.push(
        ...
        `X: <your random percentage in [1, 3], e.g. 2.0>`,
